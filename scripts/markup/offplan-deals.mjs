...
export const OFFPLAN_MORTGAGE = {
  ...
  pre: [
    // дата в шапке прижата вправо восемью табами; плейсхолдер длиннее даты и
    // переносится на вторую строку слева. Табы убираем, абзац выравнивается по
    // правому краю отдельным шагом (paragraphStyles в markup-offplan-mortgage.mjs)
