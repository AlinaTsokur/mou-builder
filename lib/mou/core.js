...
export function depositHolder(side, data) {
  const sellerAgent = !!data.sellerAgentEnabled;
  const buyerAgent = !!data.buyerAgentEnabled;
  // Депозит Покупателя обеспечивает Продавцу, что покупатель не соскочит,
  // поэтому чек держит агентство Продавца — всегда, а не только когда реквизиты
  // чека неизвестны. Раньше держатель зависел от заполнения поля в форме:
  // так было написано в шаблоне, где два абзаца про депозит расходились между
  // собой. Решение Алины 30.08.2026 — держатель один во всех случаях.
