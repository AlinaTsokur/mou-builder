...
// Ипотечный шаблон: ADM Fee — это ровно 2% от Selling Price или оценка ADM,
...
export function formForTemplate(form, template) {
  let out = template?.mortgage ? { ...form, admAdminFee: "" } : form;
...
export function buildPreview(form, rules = DEFAULT_RULES, template) {
  ...
  const validation = validateMou(data, { mortgage: !!template?.mortgage });
