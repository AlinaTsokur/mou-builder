...
export function calculate(data) {
  ...
  // ("being an amount equal to the Security Deposit"). Оба выключены → "".
  const buyerLiquidatedDamages = buyerDepositAmount !== "" ? buyerDepositAmount : sellerDepositAmount;
  const sellerLiquidatedDamages = sellerDepositAmount !== "" ? sellerDepositAmount : buyerDepositAmount;
...
function isDelayedTiming(value) {
...
export function depositHolder(side, data) {
  ...
  if (sellerAgent && buyerAgent) {
    if (side === "Buyer") {
      // Как в шаблоне: чек без реквизитов (Delayed) держит агентство продавца,
      // чек с реквизитами — агентство покупателя.
      return isDelayedTiming(data.buyerChequeTiming)
        ? "The Seller’s Agency as stakeholder"
        : "The Buyer’s Agency as stakeholder";
    }
    return "The Seller’s Agency as stakeholder";
  }
  if (sellerAgent) return "The Seller’s Agency as stakeholder";
  if (buyerAgent) return "The Buyer’s Agency as stakeholder";
...
function signatureBlock(partyList, date) {
  return (partyList || [])
    .map((p) => (p.hasPoa && p.poaName ? p.poaName : p.name))
    .filter(Boolean)
    .map((name) => `Name: ${name} Signature: ________________\nDate: ${date || ""}`)
    .join("\n\n");
}
...
export function buildReplacementsV2(data, calc, articleNumbers) {
  ...
  r.seller_signature_block = signatureBlock(data.sellers, data.sellerSignatureDate || data.agreementDate);
  r.buyer_signature_block = signatureBlock(data.buyers, data.buyerSignatureDate || data.agreementDate);
  ...
  for (const side of ["buyer", "seller"]) {
    ...
    r[`${side}_cheque_date`] = data[`${p}ChequeDate`] || "";
