...
import { articleSummary, buildArticleNumbers, DEFAULT_RULES } from "./articles.js";
...
export function normalizeForm(form = {}) {
  ...
  return {
    ...
    manualAmountToSeller: s(form.manualAmountToSeller),
    ...
    includeArticle9: boolDefault(form.includeArticle9, true),
...
export function calculate(data) {
  ...
  const remainingBalancePercent =
    data.transferThresholdPercent !== "" ? 100 - data.transferThresholdPercent : "";
  const requiredThresholdAmount =
    data.originalPrice !== "" && data.transferThresholdPercent !== ""
      ? (data.originalPrice * data.transferThresholdPercent) / 100
      : "";
  const thresholdTopUpAmount =
    data.thresholdTopUpAmount !== ""
      ...
      : requiredThresholdAmount !== "" && data.paidAmountToDeveloper !== ""
        ...
        : "";
  const remainingDeveloperBalance =
    data.remainingDeveloperBalance !== ""
      ...
      : data.originalPrice !== "" && data.paidAmountToDeveloper !== "" && thresholdTopUpAmount !== ""
        ...
        : "";
  const amountToSeller =
    String(data.manualAmountToSeller || "").toLowerCase() === "yes"
      ? data.amountToSeller
  ...
  return {
    ...
    amountToSeller,
    buyerDepositAmount,
    buyerDeposit80: buyerDepositAmount !== "" ? buyerDepositAmount * 0.8 : "",
    buyerDeposit20: buyerDepositAmount !== "" ? buyerDepositAmount * 0.2 : "",
    sellerDepositAmount,
    sellerDeposit80: sellerDepositAmount !== "" ? sellerDepositAmount * 0.8 : "",
    sellerDeposit20: sellerDepositAmount !== "" ? sellerDepositAmount * 0.2 : "",
...
export function buildReplacements(data, calc, articleNumbers) {
  ...
  r.seller_outstanding_noc_phrase = shouldUseNoc(data) ? " or No Objection Certificate (NOC)" : "";
...
export function buildDepositArticleBlock(side, data, calc) {
  ...
  const amount = calc[`${p}DepositAmount`];
  if (amount === "") return "";
  const firstWord = side === "Buyer" ? "Upon signing this agreement," : "Similarly, upon signing this agreement,";
...
export function buildDepositReturnBlock(data) {
  if (data.buyerDepositEnabled && data.sellerDepositEnabled) {
    return "Upon successful completion of the transfer of ownership on the Transfer Date, the Security Deposit cheques shall be returned to the Buyer and to the Seller or cancelled and shall not be presented for payment.";
  }
  if (data.buyerDepositEnabled) {
    return "Upon successful completion of the transfer of ownership on the Transfer Date, the Buyer’s Security Deposit cheque shall be returned to the Buyer or cancelled and shall not be presented for payment.";
  }
  if (data.sellerDepositEnabled) {
    return "Upon successful completion of the transfer of ownership on the Transfer Date, the Seller’s Security Deposit cheque shall be returned to the Seller or cancelled and shall not be presented for payment.";
  }
  return "";
}
...
export function validateMou(data) {
  ...
  if (!data.includeArticle6 && (data.includeArticle7 || data.includeArticle8 || data.includeArticle9)) {
    warnings.push("Article 6 отключена: проверьте, что ссылки на security deposit в статьях 7/8/9 уместны для этой сделки.");
  }
...
export function buildPreview(form, rules = DEFAULT_RULES) {
  ...
  const articles = articleSummary(data, rules);
