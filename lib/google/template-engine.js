...
function tabContents(doc) {
  return doc.tabs?.length
    ? doc.tabs.map((tab) => tab.documentTab?.body?.content || [])
    : [doc.body?.content || []];
}
...
function collectParagraphs(doc) {
  const out = [];
  for (const content of tabContents(doc)) {
    walkParagraphs(content, segmentEnd(content), out);
  }
  return out;
}

function walkParagraphs(content, segmentEndIndex, out) {
    ...
    if (item.paragraph) {
      ...
      for (const row of item.table.tableRows || []) {
        for (const cell of row.tableCells || []) {
          ...
          walkParagraphs(cell.content || [], segmentEnd(cell.content || []), out);
        }
      }
    }
  }
...
function collectTables(doc) {
  const out = [];
  for (const content of tabContents(doc)) walkTables(content, out);
  return out;
}
