...
export function buildConditionalTextRequests(doc, data) {
  ...
  if (!data.buyerAgentEnabled && !data.sellerAgentEnabled) {
    ...
    if (!data.sellerAgentEnabled) {
      // Agency fee line is inside a table cell, so replaceAllText is used to clear it.
      // Note: this runs AFTER placeholder substitution, so we search by stable text parts.
      requests.push({
        replaceAllText: {
          containsText: { text: "(VAT inclusive) to be paid by the Seller to", matchCase: true },
          replaceText: "",
        },
      });
      requests.push({
    ...
    if (!data.buyerAgentEnabled) {
      ...
      requests.push({
        replaceAllText: {
          containsText: { text: "(VAT inclusive) to be paid by the Buyer to", matchCase: true },
          replaceText: "",
        },
      });
      requests.push({
