...
export function buildConditionalTextRequests(doc, data) {
  ...
  if (!data.buyerAgentEnabled && !data.sellerAgentEnabled) {
    ...
    if (!data.sellerAgentEnabled) {
      ...
      requests.push({
        replaceAllText: {
          containsText: { text: "SELLER’S AGENCY\nCompany: {{seller_agent_name}}\nRepresented by: ________________________________\nSignature: ________________________________\nCompany Stamp", matchCase: true },
          replaceText: "",
        },
      });
      // Fallback in case they are separate paragraphs
      requests.push(...buildDeleteParagraphRequests(doc, (text) =>
        text.includes("SELLER’S AGENCY") || text.includes("Company: {{seller_agent_name}}") || (text.includes("Company Stamp") && !text.includes("BUYER'S"))
      ));
    }
    if (!data.buyerAgentEnabled) {
      ...
      requests.push({
        replaceAllText: {
          containsText: { text: "BUYER’S AGENCY\nCompany: {{buyer_agent_name}}\nRepresented by: ________________________________\nSignature: ________________________________\nCompany Stamp", matchCase: true },
          replaceText: "",
        },
      });
      // Fallback
      requests.push(...buildDeleteParagraphRequests(doc, (text) =>
        text.includes("BUYER’S AGENCY") || text.includes("Company: {{buyer_agent_name}}") || (text.includes("Company Stamp") && !text.includes("SELLER'S"))
      ));
    }
  }
  ...
  if (!data.buyerAgentEnabled && !data.sellerAgentEnabled) {
    ...
    // Addendum and Property Sale Authorization Form
    requests.push(...buildDeleteParagraphRequests(doc, (text) =>
      text.includes("Addendum - any addendum attached to this MOU is an integral part of the Agreement between the Parties and the Agency.")
    ));
    requests.push(...buildDeleteParagraphRequests(doc, (text) =>
      text.includes("Property Sale Authorization Form - The Agency form signed between the Seller and the Agency")
    ));
    // Deposit Release
    requests.push({
      replaceAllText: {
    ...
    requests.push({
      replaceAllText: {
        ...
        replaceText: "the Security Deposit shall remain held securely",
      },
    });
    requests.push({
      replaceAllText: {
